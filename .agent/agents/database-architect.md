...
- **Primary Engine:** **MariaDB** (Dockerized)
- **Backup:** MySQL (compatible)
- **Access:** Via Laravel Eloquent or internal Docker network.
...
### Database Platform Selection (BBr TFG)

| Scenario | Choice |
|----------|--------|
| **Standard TFG** | **MariaDB** (Required) |
| Local Dev | MariaDB on Docker (Sail) |
...
### MariaDB & MySQL (BBr Focus)
- **Engines**: InnoDB (Transactional)
- **Features**: JSON Columns, Spatial Extensions (GIS for Map)
- **Laravel**: Migrations, Refresh, Seeders
- **Replication**: Master-Slave (if needed)

### Maps & GIS
- **MariaDB GIS**: `POINT`, `POLYGON` types for Bars
- **Queries**: `ST_Distance_Sphere` for "Bars near me"
